...
abstract class BaseTeacherPeer {
	...
	const CLASS_DEFAULT = 'lib.model.Teacher';
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		{
		  call_user_func($sf_hook, 'BaseTeacherPeer', $criteria, $con);
		}
	 ...
	 *
	 * @param      Teacher $value A Teacher object.
	 * @param      string $key (optional) key to use for instance map (for performance boost if key was already calculated externally).
	 */
	public static function addInstanceToPool(Teacher $obj, $key = null)
	{
	...
	{
		// invalidate objects in LectureSchedulePeer instance pool, since one or more of them may be deleted by ON DELETE CASCADE rule.
	...
	{
		return $withPrefix ? TeacherPeer::CLASS_DEFAULT : TeacherPeer::OM_CLASS;
	}

	/**
	 * Method perform an INSERT on the database, given a Teacher or Criteria object.
	 *
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseTeacherPeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseTeacherPeer', $values, $con))
      {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseTeacherPeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseTeacherPeer', $values, $con, $pk);
    }
	...
	/**
	 * Method perform an UPDATE on the database, given a Teacher or Criteria object.
	 *
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseTeacherPeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseTeacherPeer', $values, $con))
      {
        return $sf_hook_retval;
		...
		if ($values instanceof Criteria) {
			...
			$selectCriteria->add(TeacherPeer::ID, $criteria->remove(TeacherPeer::ID), $comparison);
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseTeacherPeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseTeacherPeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the teacher table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			$con->beginTransaction();
			$affectedRows += TeacherPeer::doOnDeleteCascade(new Criteria(TeacherPeer::DATABASE_NAME), $con);
			$affectedRows += BasePeer::doDeleteAll(TeacherPeer::TABLE_NAME, $con);
	...
	/**
	 * Method perform a DELETE on the database, given a Teacher or Criteria object OR a primary key value.
	 *
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		try {
			...
			// for more than one table or we could emulating ON DELETE CASCADE, etc.
			$con->beginTransaction();
			$affectedRows += TeacherPeer::doOnDeleteCascade($criteria, $con);
	...
	protected static function doOnDeleteCascade(Criteria $criteria, PropelPDO $con)
	{
		...
		$objects = TeacherPeer::doSelect($criteria, $con);
		foreach ($objects as $obj) {


			// delete related LectureSchedule objects
			$criteria = new Criteria(LectureSchedulePeer::DATABASE_NAME);
			
			$criteria->add(LectureSchedulePeer::TEACHER_ID, $obj->getId());
			$affectedRows += LectureSchedulePeer::doDelete($criteria, $con);
		}
		return $affectedRows;
	}
	 ...
	 */
	public static function doValidate(Teacher $obj, $cols = null)
	{
		$columns = array();
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BaseTeacherPeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
